...
var lengthOfSequence = function (arr, n) {
    return arr.filter(item => item === n).length != 2 ? 0 : arr.slice(arr.indexOf(n), arr.lastIndexOf(n) + 1).length
};


var lengthOfSequence = function (arr, n) {
    return arr.filter(item => item === n).length != 2 ? 0 : arr.slice(arr.indexOf(n), arr.lastIndexOf(n)).length + 1
};